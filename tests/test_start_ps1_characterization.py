...
pytest-driven subprocess harness, because pytest itself pipes stdout/stderr
of the ``pwsh`` process we spawn. What IS tested here is the closest
...
pytestmark = pytest.mark.skipif(
    PWSH is None, reason="no pwsh/powershell executable found on PATH"
)
...
def sandbox(tmp_path):
    ...
    box.install_stub("backlogit", BACKLOGIT_STUB)
    box.install_stub("engram", ENGRAM_STUB)
...
def test_env_local_no_clobber_pre_set_process_var_wins(sandbox):
    sandbox.write_env_local("MY_CUSTOM_VAR=from_file\n")
    result = sandbox.run(
        extra_env={
            "STUB_GH_TOKEN": "tok",
...
def test_copilot_home_and_engram_data_dir_default_to_workspace_subdirs(sandbox):
    result = sandbox.run(extra_env={"STUB_GH_TOKEN": "tok"})
    assert result.returncode == 0, result.stderr
    env = sandbox.copilot_env()
    assert env["COPILOT_HOME"] == str(sandbox.workspace / ".copilot")
    assert env["ENGRAM_DATA_DIR"] == str(sandbox.workspace / ".engram")


def test_copilot_home_and_engram_data_dir_honor_preset_values(sandbox):
    result = sandbox.run(
        extra_env={
            "STUB_GH_TOKEN": "tok",
...
def test_github_token_resolved_only_when_unset_via_guarded_gh(sandbox):
    result = sandbox.run(extra_env={"STUB_GH_TOKEN": "guarded-token"})
    assert result.returncode == 0, result.stderr
    env = sandbox.copilot_env()
    assert env["GITHUB_TOKEN"] == "guarded-token"


def test_github_token_preset_is_not_overwritten_and_gh_called_once(sandbox):
    result = sandbox.run(
    ...
    assert len(sandbox.calls_log("gh")) == 1


def test_github_personal_access_token_assigned_unconditionally_and_unguarded(sandbox):
    result = sandbox.run(extra_env={"STUB_GH_TOKEN": "pat-token"})
    assert result.returncode == 0, result.stderr
    env = sandbox.copilot_env()
    assert env["GITHUB_PERSONAL_ACCESS_TOKEN"] == "pat-token"


def test_github_token_guard_is_non_fatal_when_gh_auth_token_fails(sandbox):
    ...
    result = sandbox.run(extra_env={"STUB_GH_FAIL": "1"})
    assert result.returncode == 0, result.stderr
    env = sandbox.copilot_env()
...
def test_gh_entirely_absent_is_a_non_terminating_error_script_continues(sandbox):
    ...
    (sandbox.stub_dir / "gh.cmd").unlink()
    result = sandbox.run()
    assert result.returncode == 0, result.stderr
...
def test_copilot_exe_path_takes_precedence(sandbox):
    fake_exe = sandbox.stub_dir / "custom_copilot.cmd"
    fake_exe.write_text(COPILOT_STUB, encoding="utf-8")
    result = sandbox.run(
        extra_env={
            "STUB_GH_TOKEN": "tok",
            "COPILOT_EXE_PATH": str(fake_exe),
            "COPILOT_EXE": str(sandbox.stub_dir / "other_copilot.cmd"),
        }
    )
    assert result.returncode == 0, result.stderr
...
def test_copilot_exe_env_var_used_when_exe_path_unset(sandbox):
    fake_exe = sandbox.stub_dir / "custom_copilot2.cmd"
    fake_exe.write_text(COPILOT_STUB, encoding="utf-8")
    (sandbox.stub_dir / "copilot.cmd").unlink()  # remove PATH resolution candidate
    result = sandbox.run(extra_env={"STUB_GH_TOKEN": "tok", "COPILOT_EXE": str(fake_exe)})
    assert result.returncode == 0, result.stderr
    assert sandbox.copilot_env() is not None


def test_copilot_resolved_from_path_when_no_explicit_vars_set(sandbox):
    result = sandbox.run(extra_env={"STUB_GH_TOKEN": "tok"})
    assert result.returncode == 0, result.stderr
    assert sandbox.copilot_env() is not None


def test_copilot_unresolvable_raises_actionable_error(sandbox):
    (sandbox.stub_dir / "copilot.cmd").unlink()
    result = sandbox.run(extra_env={"STUB_GH_TOKEN": "tok"})
...
def test_remote_flag_appended_when_use_remote_truthy(sandbox, value):
    result = sandbox.run(argv=["foo"], extra_env={"STUB_GH_TOKEN": "tok", "COPILOT_USE_REMOTE": value})
    assert result.returncode == 0, result.stderr
    assert sandbox.copilot_args() == "--remote foo"


@pytest.mark.parametrize("value", ["false", "0", "yes", ""])
def test_remote_flag_not_appended_when_use_remote_not_truthy(sandbox, value):
    result = sandbox.run(argv=["foo"], extra_env={"STUB_GH_TOKEN": "tok", "COPILOT_USE_REMOTE": value})
    assert result.returncode == 0, result.stderr
    assert sandbox.copilot_args() == "foo"


def test_remote_flag_not_duplicated_when_operator_already_passed_it(sandbox):
    result = sandbox.run(
...
def test_operator_argv_forwarded_verbatim(sandbox):
    result = sandbox.run(argv=["alpha", "--flag", "value123"], extra_env={"STUB_GH_TOKEN": "tok"})
    assert result.returncode == 0, result.stderr
    assert sandbox.copilot_args() == "alpha --flag value123"
...
@pytest.mark.parametrize("child_exit_code", ["0", "3", "7"])
def test_pwsh_host_exit_code_does_not_mirror_child_exit_code(sandbox, child_exit_code):
    result = sandbox.run(extra_env={"STUB_GH_TOKEN": "tok", "STUB_COPILOT_EXIT_CODE": child_exit_code})
...
def test_backlogit_sync_runs_when_resolved(sandbox):
    result = sandbox.run(extra_env={"STUB_GH_TOKEN": "tok"})
    assert result.returncode == 0, result.stderr
    calls = sandbox.calls_log("backlogit")
    assert any("sync" in c for c in calls)


def test_backlogit_sync_failure_is_non_fatal(sandbox):
    ...
    result = sandbox.run(extra_env={"STUB_GH_TOKEN": "tok", "STUB_BACKLOGIT_FAIL": "1"})
    assert result.returncode == 0, result.stderr
    assert sandbox.copilot_env() is not None
    calls = sandbox.calls_log("backlogit")
...
def test_backlogit_absent_is_skipped_silently(sandbox):
    (sandbox.stub_dir / "backlogit.cmd").unlink()
    result = sandbox.run(extra_env={"STUB_GH_TOKEN": "tok"})
    assert result.returncode == 0, result.stderr
    assert sandbox.copilot_env() is not None


def test_engram_direct_prewarm_happy_path(sandbox):
    result = sandbox.run(extra_env={"STUB_GH_TOKEN": "tok"})
    assert result.returncode == 0, result.stderr
    calls = sandbox.calls_log("engram")
    assert len(calls) == 1
    assert "--direct" in calls[0]


def test_engram_direct_failure_falls_back_to_bind_and_daemon_sync(sandbox):
    result = sandbox.run(extra_env={"STUB_GH_TOKEN": "tok", "STUB_ENGRAM_DIRECT_FAIL": "1"})
    assert result.returncode == 0, result.stderr
    calls = sandbox.calls_log("engram")
    assert len(calls) == 3
    assert "--direct" in calls[0]
...
def test_engram_fallback_failure_is_non_fatal(sandbox):
...
def test_engram_absent_is_skipped_silently(sandbox):
    (sandbox.stub_dir / "engram.cmd").unlink()
    result = sandbox.run(extra_env={"STUB_GH_TOKEN": "tok"})
    assert result.returncode == 0, result.stderr
    assert sandbox.copilot_env() is not None
...
def test_final_invocation_line_has_no_stdio_redirection(sandbox):
...
def test_child_stdio_is_not_intercepted_functionally(sandbox):
