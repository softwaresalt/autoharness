...
def backlog_dir(tmp_path: Path) -> Path:
    root = tmp_path / ".backlogit"
    (root / "queue").mkdir(parents=True)
    (root / "archive").mkdir(parents=True)
    return root


def test_fully_covered_root_feature_selects_cascade(backlog_dir: Path) -> None:
    _write_artifact(backlog_dir, "queue", "200-F", "feature")
    _write_artifact(backlog_dir, "queue", "200.001-T", "task", parent_id="200-F")
    _write_artifact(backlog_dir, "queue", "200.002-T", "task", parent_id="200-F")

    decision = classify_shipment_close_path(
        ["200-F", "200.001-T", "200.002-T"], backlog_dir
    )
...
def test_verified_childless_terminal_root_feature_also_qualifies(backlog_dir: Path) -> None:
    ...
    _write_artifact(backlog_dir, "queue", "201-F", "feature")

    decision = classify_shipment_close_path(["201-F"], backlog_dir)

    assert decision.close_path is ClosePath.CASCADE
    assert decision.qualifying_feature_ids == ("201-F",)


def test_root_feature_missing_child_falls_back_to_safe_close(backlog_dir: Path) -> None:
    _write_artifact(backlog_dir, "queue", "202-F", "feature")
    _write_artifact(backlog_dir, "queue", "202.001-T", "task", parent_id="202-F")
    _write_artifact(backlog_dir, "queue", "202.002-T", "task", parent_id="202-F")

    # 202.002-T is a real child of 202-F but is NOT listed in the manifest.
    decision = classify_shipment_close_path(["202-F", "202.001-T"], backlog_dir)

    assert decision.close_path is ClosePath.SAFE_CLOSE
    assert "202.002-T" in decision.reason


def test_non_root_feature_member_falls_back_to_safe_close(backlog_dir: Path) -> None:
    _write_artifact(backlog_dir, "queue", "203-F", "feature")
    # 204-F declares a parent -- it is not a root feature.
    _write_artifact(backlog_dir, "queue", "204-F", "feature", parent_id="203-F")

    decision = classify_shipment_close_path(["204-F"], backlog_dir)
...
def test_extra_task_belonging_to_no_member_feature_falls_back_to_safe_close(
    backlog_dir: Path,
) -> None:
    _write_artifact(backlog_dir, "queue", "205-F", "feature")
    _write_artifact(backlog_dir, "queue", "205.001-T", "task", parent_id="205-F")
    # 206.001-T is an orphaned manifest member: its parent is not any
    # feature member of this manifest.
    _write_artifact(backlog_dir, "queue", "206.001-T", "task", parent_id="206-F")

    decision = classify_shipment_close_path(
        ["205-F", "205.001-T", "206.001-T"], backlog_dir
    )
...
def test_childlessness_query_failure_falls_back_to_safe_close_not_treated_as_childless(
    backlog_dir: Path,
) -> None:
    _write_artifact(backlog_dir, "queue", "207-F", "feature")
    ...
    (backlog_dir / "queue" / "garbage.md").write_text(
        "this is not a valid backlog artifact file\nno frontmatter delimiters here\n",
        encoding="utf-8",
    )

    decision = classify_shipment_close_path(["207-F"], backlog_dir)

    assert decision.close_path is ClosePath.SAFE_CLOSE
    assert "could not be verified" in decision.reason


def test_adding_child_to_previously_qualifying_childless_feature_flips_to_safe_close(
    backlog_dir: Path,
) -> None:
    ...
    _write_artifact(backlog_dir, "queue", "208-F", "feature")

    before = classify_shipment_close_path(["208-F"], backlog_dir)
    assert before.close_path is ClosePath.CASCADE

    _write_artifact(backlog_dir, "queue", "208.001-T", "task", parent_id="208-F")

    after = classify_shipment_close_path(["208-F"], backlog_dir)
    assert after.close_path is ClosePath.SAFE_CLOSE
    assert "208.001-T" in after.reason


def test_manifest_with_no_feature_member_falls_back_to_safe_close(backlog_dir: Path) -> None:
    ...
    _write_artifact(backlog_dir, "queue", "209-F", "feature")
    _write_artifact(backlog_dir, "queue", "209.001-T", "task", parent_id="209-F")

    decision = classify_shipment_close_path(["209.001-T"], backlog_dir)

    assert decision.close_path is ClosePath.SAFE_CLOSE
    assert "no feature member" in decision.reason


def test_manifest_item_that_cannot_be_found_falls_back_to_safe_close(backlog_dir: Path) -> None:
    decision = classify_shipment_close_path(["999-F"], backlog_dir)

    assert decision.close_path is ClosePath.SAFE_CLOSE
    assert "999-F" in decision.reason


def test_empty_manifest_falls_back_to_safe_close(backlog_dir: Path) -> None:
