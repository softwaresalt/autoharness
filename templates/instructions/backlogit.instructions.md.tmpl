...
1. Prefer targeted query operations over reading many `.backlog/` markdown files directly (`.backlog` is the default for new installs; legacy `.backlogit/` remains supported, and both roots present at once must fail closed until the operator resolves the ambiguity).
...
If `.backlog/` or legacy `.backlogit/` content was edited outside the usual backlogit mutation flow (and if both roots are present, fail closed until the operator resolves the ambiguity), refresh the index
