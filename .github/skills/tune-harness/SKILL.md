...
* **Pipeline-topology hook drift** (universal — when the installed `autoharness`
  CLI provides `gate pipeline-topology`):
  ...
  2. Verify `scripts/pre-commit-pipeline-topology.sh` and `.ps1` exist and
  ...
  3. Verify both hook families remain advisory-first by default (no
     unconditional `exit 1`/blocking without the `AUTOHARNESS_TOPOLOGY_GATE_BLOCKING`
     toggle) and contain no retry loop; flag as P1 Degrading if a hook was
     hardened to hard-fail-by-default or grew a retry loop
  4. If all checks pass, classify as `unchanged` or `user-modified` (checksum comparison)
