...
* **Copilot code-review focus instruction**: When the workspace is hosted on GitHub, verify `copilot-code-review.instructions.md` is present in the instructions directory and still carries `applyTo: '**'`, `excludeAgent: 'cloud-agent'`, the FOCUS-on-high-value-concerns framing, the de-prioritize-harness-enforced section, and the weakened-enforcement review guidance. Flag as P1 Degrading drift if the file is missing, if `excludeAgent` was changed to `code-review` (which would wrongly silence the reviewer) or `coding-agent` (invalid — GitHub accepts only `code-review` or `cloud-agent`), or if the FOCUS framing degraded into a blanket-suppression instruction. Refresh via generate-when-absent / back-up-and-refresh-when-diverged, and preserve intentional operator edits (respect `.autoharness/drift-ignore`) rather than clobbering them. **Base-branch activation caveat**: GitHub Copilot code review reads instruction files from the pull request's base branch, so a refreshed instruction only governs code review after the tuning pull request is merged to the base branch — the tuning pull request itself is validated structurally only, and the Settings → Copilot → Code review custom-instructions toggle is an advisory prerequisite, not a hard tuning gate.
...
3. The instruction contains the Checkpoint-Recovery / Prune-on-Restore Protocol section (prune allowlist: active cursor, unresolved-checkpoint pointer, gate verdicts never pruned; engram-unreachable → operator handoff, no prune/resume).
4. `_orchestrator.agent.md` contains its Crash-Resumption Protocol step (zero-candidate no-op; explicit unique checkpoint selection; CheckpointV1 `agent`-ownership validation; owner-exclusive routing — never performs Stage/Ship-owned restore/resume/prune itself).
5. `_stage.agent.md` and `_ship.agent.md` each contain their own Crash-Resumption / Startup Recovery Protocol section (zero-candidate normal startup; explicit operator filename selection; owner validation against the agent's own role; owner-scoped resolution only after a confirmed successful owner resume; fail closed with no fresh-start fallback on invalid/ambiguous checkpoint reads).
