...
* **Source artifact cleanup** — the traceable record of which source artifacts feeding the shipped scope were retired. For every shipped top-level item (feature or chore), record the `custom_fields.source_stash_id` and `custom_fields.source_deliberation_id` values, each with its outcome: archived, skipped because it was already archived, skipped because it was not found, or `none` when the item declares no such field. Ship performs the retirement against the backlog artifacts under `.backlogit/` during post-merge closure and reports the resulting IDs here, so this section — not the tool's own logs — is the closure artifact's system of record for source-artifact retirement.
