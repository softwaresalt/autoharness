...
| `{{HARNESS_ENFORCED_SUMMARY}}` | Synthesized bullet list of mechanical/process concerns the installed composition already enforces (policies, gates, linters, review personas). Used only by `copilot-code-review.instructions.md`. Never leave unresolved — when the composition is minimal, fall back to the conservative generic default below | `* Formatting and linting`<br>`* Commit/branch mechanics`<br>`* Test-first red-green gates`<br>`* Merge/PR hygiene`<br>`* Backlog bookkeeping` | (same generic default) | (same generic default) |
