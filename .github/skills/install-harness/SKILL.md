...
| `{{STAGE_FAMILY}}` | `config.model_routing.stage.model_family`, fallback `{{TIER_3_FAMILY}}` | `claude-opus-4.8` | Model family for the Stage role route (P-013.5); falls back to Tier 3 when the `stage` route or this sub-field is unset |
...
| `{{ESCALATION_REASONING_EFFORT}}` | `config.model_routing.escalation.reasoning_effort`, fallback `{{TIER_3_REASONING_EFFORT}}` | _(empty)_ | Reasoning effort for the telemetry-driven auto-escalation route (P-013.6); falls back to Tier 3 per-field when the `escalation` route or this sub-field is unset |
| `{{ESCALATION_PROVIDER}}` | `config.model_routing.escalation.model_provider`, fallback `{{TIER_3_PROVIDER}}` | _(empty)_ | Model provider for the auto-escalation route (P-013.6); falls back to Tier 3 per-field when the `escalation` route or this sub-field is unset |
| `{{ESCALATION_FAMILY}}` | `config.model_routing.escalation.model_family`, fallback `{{TIER_3_FAMILY}}` | `claude-opus-4.8` | Model family for the auto-escalation route (P-013.6); falls back to Tier 3 per-field when the `escalation` route or this sub-field is unset. **Same-route guard**: if the fully-resolved `(ESCALATION_FAMILY, ESCALATION_PROVIDER, ESCALATION_REASONING_EFFORT)` tuple equals the acting agent's own role route tuple (e.g., Stage's role route already equals tier3), the resolution is `ESCALATION_DEGRADED` (same-route no-op) and the halting agent falls back to the existing operator-halt path rather than auto-escalating to an identical model. |
...
9. **Pipeline-topology hook verification** (mixed scope: item (a) applies only when
   ...
   e. **Only when `ci.platform` is GitHub Actions and the CI workflow row applies**:
      confirm `scripts/ci-topology-check.sh` exists, invokes
      ...
      `PIPELINE_TOPOLOGY_GATE_REQUIRED` repository variable. Report FAIL for
      a missing entrypoint, a shipment-scoped invocation, an added
      advisory-degrade toggle, or a workflow job missing the toggle wiring
