...
| `{{ORCHESTRATOR_FAMILY}}` | `config.model_routing.orchestrator.model_family`, fallback `{{TIER_2_FAMILY}}`, fallback `gpt-5.4` | `gpt-5.4` | Model family for the Orchestrator; falls back to Tier 2 default, then to `gpt-5.4` if tier2 is also unset |
