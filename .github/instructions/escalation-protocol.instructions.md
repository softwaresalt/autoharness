...
## Escalation Route Resolution (P-013.6)

This workspace declares an explicit `config.model_routing.escalation` route:
`model_family = gpt-5.6-sol`, `model_provider = openai`,
`reasoning_effort = high` — a distinct vendor/family from the Stage role
route (`claude-opus-4.8`/`anthropic`), so an auto-escalation attempt is a
genuine escalation, not a same-route no-op. When the `escalation` route or a
given sub-field is absent or empty, resolution falls back **per field** to
`model_routing.tier3` (`claude-opus-4.8`; this workspace's `tier3` is a plain
model-identifier string, so its provider/reasoning-effort sub-fields resolve
empty via fallback). This mirrors the P-013.5 `stage`/`ship` role-route
fallback pattern: a targeted override, never a
parallel tier taxonomy.
...
3. **Same-route no-op.** The fully-resolved escalation tuple
   `(model_family, model_provider, reasoning_effort)` equals the acting agent's
   own already-resolved role route tuple (P-013.5) for this session — for
   example, an agent whose role route is already pinned to `tier3` receiving an
   unset `escalation` route that also falls back to `tier3` would "escalate" to
   an identical model, which is not a real escalation.
