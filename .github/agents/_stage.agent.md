...
1. Call `backlogit_list_checkpoints` with `consumer_id: "stage"` and `status: "active"` (no age bound — an unresolved active checkpoint remains a candidate regardless of age, since age alone can never prove a prior session dead), then filter the result to entries whose `agent` field is exactly `stage` (Stage's own candidates only). Stale-checkpoint cleanup is a separate, explicit hygiene operation and never a filter on candidate enumeration here.
2. If NO active `stage`-owned checkpoint exists, there is nothing to recover. Continue directly with normal stash triage. This is EXPLICITLY NOT a failure and NOT an operator handoff — it is the expected steady state on most session starts.
