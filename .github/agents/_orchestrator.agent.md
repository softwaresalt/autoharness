...
1. **Enumerate recovery candidates**: list unresolved/`active` checkpoints via the registered checkpoint tool (backlogit `list_checkpoints status=active`, or the configured registry equivalent).

2. **Zero-candidate case (expected steady state)**: if NO active recovery candidate exists, there is nothing to recover. Continue directly to Step 0 State Assessment. Zero candidates is EXPLICITLY NOT a failure and NOT an operator handoff — it is the normal, expected state on almost every session start.

3. **One or more candidates exist**: the recovery contract engages only now. More than one checkpoint may be active concurrently across agents — a `stage`-owned and a `ship`-owned checkpoint can both be active at the same time — so the Orchestrator NEVER auto-picks. Present the full list of active candidates (filename, `agent`, `session_id`, `phase`, `created_at`, `shipment_id`/`feature_id`, `resume_hint`) to the operator and REQUIRE EXPLICIT OPERATOR SELECTION of a SINGLE checkpoint by filename before any further action.

4. **Ownership validation**: once a checkpoint is selected, validate its CheckpointV1 `agent` field. Per the backlogit schema this field is `required,oneof=ship stage` — it MUST be exactly `stage` or `ship`.

5. **Owner-exclusive routing (NEVER perform owner work directly)**: route ALL restore/resume/prune work for the selected checkpoint EXCLUSIVELY to the agent that owns it:
