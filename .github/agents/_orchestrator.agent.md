...
1. Select the next `queued` shipment by **queue ordering**, not priority alone:
   * **First-pass candidate source**: run `backlogit_list_shipments` filtered to `queued` to list queued shipments in execution order — ordered by the backlog tool's queue position, then priority. Per the canonical shipment lifecycle it withholds shipments parked at `status: blocked`. Treat it as an ordering aid and first filter — **not** the sole eligibility authority. Keep this rule expressed in registry-resolved operations so non-backlogit installs stay valid; when backlogit is the configured tool, the concrete queue/dependency recipe (`queue view`, `item_deps`, `queue_position`, `dep add --type blocks`) lives in the backlogit **Shipment Sequencing Protocol**.
