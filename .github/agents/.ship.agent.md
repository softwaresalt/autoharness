...
1. **Close the shipment via single-artifact safe-close (NEVER the cascade `backlogit_ship_shipment`, P-015)**:
   Delegate to the `shipment-reconcile` skill's safe-close procedure. Do NOT call
   ...
   a. Read the shipment manifest: `backlogit shipment get <shipment_id>`. The
      `custom_fields.items` list is the ONLY set of IDs that may be archived.
   b. **Partial-feature detection**: derive the covering feature ID (e.g. tasks `055.002-T`
      belong to feature `055-F`) and compute the **protected set** — the parent feature file
      plus every sibling task under it that is NOT in the manifest. Every protected-set file
      must remain in `.backlogit/queue/` after closure.
   c. **Archive each manifest item individually** (manifest IDs only): for each item,
      `backlogit move <id> --status done` then `backlogit archive <id>`. Skip any item whose
      file is already in `.backlogit/archive/` (pre-archived) to avoid false-positive cascade
      flags.
   d. **Verify-after-each invariant**: after each archival, run
      `git status --short -- .backlogit/` and confirm no protected-set file was moved into
      `.backlogit/archive/` or deleted from `.backlogit/queue/`.
   e. **git-revert-on-cascade**: if a protected-set artifact was archived or deleted, run
   ...
   f. **Verify archive integrity (P-007)**: run `git status -- ".backlogit/archive/"`; if any
      archive files appear as working-tree deletions, run `git restore .backlogit/archive/`.
   g. **Commit only after the protected set is intact**: `git add .backlogit/` then
