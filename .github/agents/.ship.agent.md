...
4. **P-018 Copilot-Review Completion Gate (NON-NEGOTIABLE, fail-closed)**: Before
   ...
   `auto`). See `.github/instructions/github-pr-automation.instructions.md` §1.9.4
   Check 5.
...
8. Execute the merge only after receiving explicit operator approval (or a valid
   ...
   gate, re-run §1.9 AND the P-018 copilot-review gate in full before executing the
   merge, regardless of approval source.
