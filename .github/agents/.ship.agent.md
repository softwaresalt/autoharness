...
6. **Operator approval gate**: After the §1.9 gate passes and the releasability evidence is in hand, present the PR readiness summary
   ...
   and scope match.
7. Execute the merge only after receiving explicit operator approval (or a valid
   ...
   source.
8. **Dark-mode merge/admin fallback state machine (P-017)**: When `DARK_MODE_ACTIVE`
