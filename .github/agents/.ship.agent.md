...
2. Verify all tasks have clear scope and acceptance criteria. Task-only shipment
   ...
   For 092-S-style sizing/context shipments, do not treat the shipment as
   execution-ready while `079.013-T` or `079.015-T` remains incomplete; this is
   enforced through `dependencies`, not `blocked-on-external`.
...
2. **Begin telemetry context**: Immediately after claim and before pre-build
   ...
   through the task loop. If telemetry is disabled, skip context carry/record
   close for this task without failing the lifecycle or creating telemetry
   artifacts. Do not re-read backlogit size, hierarchy, or shipment membership
   after begin; the `WorkSizingSnapshot` is immutable.
   In short: skip context carry/record close when begin returns `disabled`.
...
7. **Record close telemetry**: If begin returned an enabled `context_ref`, create
   the close-time epoch payload from task roll-up metrics and run
   `autoharness telemetry record --context-ref {context_ref} --from-json {epoch_payload_path} --json`
   before or as the task is marked done. The record path preserves the same stable
   epoch_id and never re-reads backlogit size, hierarchy, or shipment membership.
