...
7. **Pre-merge strategy guardrail (P-009, NON-NEGOTIABLE)**: Before executing any
   ...
   `action: halted`), and instruct the operator to disable "Allow squash merging"
   and "Allow rebase merging" in repository settings before proceeding. When
...
8. Execute the merge only after receiving explicit operator approval (or a valid
   `DARK_MODE_ACTIVE` approval record) AND having a §1.9 gate pass on record.
   Immediately before any normal merge or admin fallback, re-query the PR
   `headRefOid` and confirm it still matches the HEAD covered by the latest
   passed §1.9 gate. If the branch HEAD advanced at any point after the §1.9
   gate, re-run §1.9 in full before executing the merge, regardless of approval
   source.
...
branch-per-release-unit principle. If direct push to `main` is blocked or the
established workspace pattern leaves backlog state as a local record, follow that
pattern; otherwise use the closure branch + PR.
