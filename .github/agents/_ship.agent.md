...
1. Call `backlogit_list_checkpoints` with `consumer_id: "ship"`, `status: "active"`, and `max_age_hours: 168`, then filter the result to entries whose `agent` field is exactly `ship` (Ship's own candidates only).
