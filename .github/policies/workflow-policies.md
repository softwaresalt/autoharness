...
**Postcondition** (harness-architect): The task has `harness-ready` label, the harness compiles (`python -m py_compile src/autoharness/cli.py`), and all tests fail with expected failure markers.
...
**Statement**: The harness-architect must confirm the red phase for the current task's generated harness tests (compilation succeeds and every generated harness test for the current task fails with its own expected marker) before the `harness-ready` label is applied. This gate is scoped to the current task's generated harness tests, never to the full historical test population that already exists in a healthy repository -- unrelated, previously established tests may continue to pass. Per P-002, only the harness-architect skill applies `harness-ready`; ship never bypasses this gate.

**Precondition**: All of the following must be true:

1. `python -m py_compile src/autoharness/cli.py` exits 0 -- compilation succeeds, including the newly generated harness tests.
2. `PYTHONPATH=src python -m unittest discover -s tests` (the canonical whole-suite test command, invoked exactly as resolved with no runner substitution and no scoped/targeted subset) exits non-zero -- because the current task's generated harness tests are red. Unrelated established tests elsewhere in the suite may pass; their passing is not a violation.
3. Every generated harness test for the current task is discovered by that run AND fails with its own expected failure marker; evaluate each such test individually rather than relying on the aggregate non-zero exit code alone.

The following outcomes are NEVER valid red-phase evidence for a generated harness test, and MUST be treated as harness defects requiring a fix before proceeding:

* Zero-discovery: the run reports zero generated harness tests collected for the current task.
* Collection/import/syntax errors: the run aborts during collection instead of exercising the generated harness tests.
* Missing, wrong, or cross-test markers: a generated harness test fails for a reason other than its own expected marker, or is credited with a different test's marker, rather than evaluated individually.
* Pass, skip, `expectedFailure`, or `unexpectedSuccess`: a generated harness test that passes, is skipped, is marked expected-to-fail, or unexpectedly succeeds counts as no red-phase observation for that test.
...
**Violation Action** (Decisions G1–G9; a `git restore` of `.backlogit/archive/` is a destructive command and MUST be gated behind a fresh, live, non-synthesizable operator approval — never issued unconditionally):

**G1 — The authorization source is a direct runtime operator approval (binding)**: authorization to run the restore MUST be a LIVE approval RESULT obtained at the moment of the request over an INDEPENDENT operator channel the executing agent CANNOT synthesize — the intercom approval/clearance operation, an interactive operator ask/confirm prompt, or the operator session channel. The defining property is non-synthesizability: the approving act must originate outside the agent's own writable surface, and the agent has no operation that can produce the affirmative result on its own behalf.

**G2 — A backlog comment is evidence only, never authority**: after (and only after) a G1 approval result is obtained, the agent SHOULD record an `APPROVAL: P-007-ARCHIVE-RESTORE` comment on the shipment item carrying the shipment ID, the exact archive paths, the operator actor identity, the approval timestamp, and the channel the approval came over. This record exists for audit and traceability only. It is NOT read back as authorization, is NOT a substitute for a live G1 result, and its presence does NOT shorten, cache, or pre-satisfy any future approval — each remediation attempt requires its own fresh G1 approval.

**G3 — No independent channel means halt, do not restore (binding)**: when NO independent approval channel is available — intercom degraded or unreachable, no interactive operator session, ask/confirm unavailable, or the channel present but unanswered — detect the violation, halt, do not run `git restore`, record it through P-005 telemetry with the exact remediation command for a human to run, and leave the working tree untouched. Absence of a channel is NEVER an implicit approval and NEVER a fallback to the comment path in G2.

**G4 — Dark-mode / AFK is fail-closed**: under dark-factory/AFK mode no operator is present to give a G1 approval, so `git restore` NEVER runs. A dark-mode run always reports and never restores. This is the Principle VII-conformant outcome, not a degradation; dark mode is the specific case of the general G3 rule, not an exception to it.

**G5 — Verification is a read of a live result, not an inference**: match the approval result to the CURRENT shipment ID and the EXACT archive paths to be restored. Mismatch, absence, ambiguity, timeout, or an unreadable channel is a REFUSAL, never a pass.
...
**G7 — Deterministic static contract (mandatory, four required test observables over this policy text)**:

1. **No-approval condition**: the remediation step contains NO unconditional `git restore` instruction — every occurrence of the restore command is either (a) inside an explicitly G1-gated branch, or (b) presented as a command FOR A HUMAN TO RUN in the refusal/report path. The default path is detect-halt-report-with-P-005-telemetry, and the tree is left untouched.
2. **Mismatch condition**: the text requires the approval result to match the current shipment ID AND the exact archive paths, and names mismatch/ambiguity/timeout/unreadable as a refusal.
3. **Self-authorization negative coverage**: the text NOWHERE treats an agent-writable artifact as an authorization source — no clause permits a backlog comment, a telemetry record, a file the agent can write, or any agent-callable operation to satisfy the gate; and the affirmative G2 clause is present stating a backlog comment is evidence only and is NOT an authorization source. Both halves are required.
4. **No-channel condition**: the text declares halt-without-restore when no independent channel is available, in ALL modes, and that dark-factory/AFK is named as an instance of that rule rather than an exception to it.

**G8 — Refusal message content**: the refusal message names the missing authorization channel and the exact command a human can run themselves, so a legitimate operator is never stranded without a remedy.

**G9 — No new approval store, file format, or CLI**: G1 names approval channels the harness already has, and G2 uses the existing tool-owned comment surface for audit only. No new approval broker, remediation executor, or CLI surface is authorized by this policy.

**Recovery procedure (executed only after a G1 approval result is obtained)**:

1. Obtain a live G1 approval result over an independent operator channel, matched to the current shipment ID and the EXACT set of deleted archive paths detected by the Required Check (G5). Absent, mismatched, ambiguous, timed out, or unreadable → refusal (go to step 6).
2. **Revalidate immediately before restoring**: re-run `git status -- ".backlogit/archive/"` again, at the moment restoration is about to begin, and compare the result to the approved path set from step 1. If any approved path no longer reports a deletion (it was recreated, modified, or otherwise changed since the approval was matched), or if the current deletion set differs at all from the approved set, the approval is STALE and MUST NOT be used — a deleted path can be recreated or modified while an approval is pending, and restoring against a stale observation could overwrite content that was never approved for deletion. On staleness, refuse and require a fresh G1 approval matched to the newly-observed state (return to step 1); never proceed to step 3 with a stale approval.
3. Build a pathspec from ONLY the revalidated paths from step 2 — never the whole `.backlogit/archive/` directory, which could discard other tracked modifications below it that were never part of the approval. Run `git restore -- <deleted-path-1> <deleted-path-2> ...` limited to that exact pathspec to recover only the deleted archive files the approval covers.
4. Re-verify with `git status -- ".backlogit/archive/"` — confirm the same paths no longer report a deletion and no other deletion remains.
5. Stage ONLY the same restored paths from step 3: `git add -- <deleted-path-1> <deleted-path-2> ...` — never an unscoped `git add .backlogit/archive/`, which could stage unrelated work under the same directory beyond what was approved. Proceed to the commit step only after verification passes. Record the `APPROVAL: P-007-ARCHIVE-RESTORE` audit comment (G2), naming the exact restored paths, after the restore succeeds.
6. If no G1 approval is obtained, the revalidation in step 2 finds the approval stale with no fresh approval subsequently obtained, or restoration fails after approval, halt and prompt the operator with the exact pathspec-scoped command above (built from the same detected deletions) to run themselves (G8). Broadcast a P-005 violation event. Do not restore.
...
**Statement**: Shipment closure is a flat-manifest operation. `manifest_scope(S)` is the manifest's explicit item IDs, `closure_scope(S) = items(S) ∪ {S}`, and ancestry never expands that closure scope. The ship agent MUST use the `shipment-reconcile` safe-close procedure as the default path: archive only the manifest's explicit item IDs, one artifact at a time, and then archive only the shipment record itself as its own single artifact. The cascade `backlogit_ship_shipment` path is forbidden by default because it treats the shipment as a proxy for a covering feature and can mutate out-of-manifest descendants; it is permitted only when the machine-checkable invariants below prove every reachable out-of-manifest descendant is engine-inert and the remainder of the contract still holds.

**Precondition**: The shipment manifest (`items`) has been read; the observation set of out-of-manifest artifacts reachable from the covering feature has been computed; and a baseline-invariance snapshot has been captured for every observed artifact outside `closure_scope(S)`, recording its baseline location plus content hash alongside `git status --short -- ".backlogit/"`. An artifact already archived, descoped, or missing at baseline is recorded as baseline state and is explicitly NOT a halt. Manifest ordering is not a closure precondition.

**Required Check (verify-after-each invariant)**: This invariant is **path-specific**, not one universal rule. On the **safe-close** path, after each mutation, compare the current state of every observed artifact outside `closure_scope(S)` against its baseline fingerprint. On the **CASCADE** path, after each mutation, compare the current state of every observed artifact outside `allowed_ids(S)` against its baseline fingerprint instead — a required transition of a `validated_linked_deliberations(S)` member (in `allowed_ids(S)` but outside `closure_scope(S)`) from its pre-close state into `archived` is an expected, in-scope cascade mutation, never itself a violation signal. In both cases, any change relative to baseline — location change, content-hash change, deletion, rename, new archive presence, or `parent_id` drift — is a cascade signal. The check watches for change; it does not require queue presence at baseline or after each step.

**Postcondition**: Safe-close archives only `closure_scope(S)`, preserves byte-identical baseline state for every artifact outside `allowed_ids(S)`, and leaves the shipment record archived with `archived_status: shipped`. When `CASCADE` is selected, the live postcondition gate remains `archived_ids ⊆ allowed_ids(S)` and `required_ids(S) ⊆ archived_ids`; no artifact outside `allowed_ids(S)` may be created, modified, moved, or deleted.

**Violation Action (approval-gated rollback)**: If any artifact outside the Required Check's applicable scope for the active path (`closure_scope(S)` for safe-close, `allowed_ids(S)` for CASCADE) changes relative to baseline, follow this exact sequence and do nothing else: capture evidence → HALT → emit P-005 → request EXPLICIT operator approval → REVALIDATE the approved paths and current state → execute ONLY the approved rollback. Automatic, immediate, and notification-only destructive revert language is prohibited.

**Relationship to P-007**: P-015 complements **P-007** (backlogit archive integrity after shipment). P-007 addresses archive-file deletion quirks during archival. P-015 constrains shipment closure so out-of-manifest artifacts remain outside the permitted mutation set and any baseline drift outside `allowed_ids(S)` halts the run.

**SUPERSESSION NOTE (2026-09-15, amended 2026-09-16).** The withdrawn hierarchical-closure reading and the withdrawn `TERMINAL_CLOSE` design are retained as **SUPERSEDED PROVENANCE ONLY**. See `docs/decisions/2026-09-15-173-s-terminal-shipment-closure-deliberation.md` for that superseded provenance. The existing 155-S supersession note below remains authoritative and unchanged.

**Evidence-class note.** CLASSIFIER LAW and ENGINE LAW are disjoint. The only held engine proposition on this surface is the path-scoped comparison between immutable commits `358b63b4` and `e4ca20e5`: `git diff --stat 358b63b4 e4ca20e5 -- .backlogit/archive/165.007-T.md .backlogit/archive/165.010-T.md` is empty, and `git rev-parse 358b63b4:<path>` / `git rev-parse e4ca20e5:<path>` return identical blob OIDs for both paths. `returned_ids` contents and `parent_id` clearing remain indicative and unproven rationale only; they may never be stated as measured fact and may never authorize `CASCADE`.

#### D1a — Scope vocabulary

| Set | Definition | Role |
|---|---|---|
| **`manifest_scope(S)`** | exactly `items(S)` | what is delivered. Membership is explicit and exhaustive. |
| **`closure_scope(S)`** | `items(S) ∪ {S}` | what this closure is about and the only artifacts it may deliberately transition. |
| **`allowed_ids(S)`** | `closure_scope(S) ∪ validated_linked_deliberations(S)` | the maximal permitted postcondition set. |
| **`required_ids(S)`** | `{ S } ∪ { qualifying feature members of S } ∪ { x ∈ allowed_ids(S) : x is not already truly archived in the pre-close snapshot }` — the shipment record and every qualifying feature member are unconditionally required regardless of their own pre-close declared status; every other `allowed_ids(S)` member (a manifest task item, or a qualifying feature member's validated linked deliberation) is required only when it was not already truly archived pre-close; see item 7 below and the `shipment-reconcile` skill's Cascade Close Sub-Procedure (step 3) for the full rationale | the minimal mandatory postcondition set. |

`validated_linked_deliberations(S)` is admitted only when all four conditions hold: (1) the record is a deliberation/decision artifact, never a backlog work item; (2) it is reached solely through the engine's linked-deliberation expansion from a manifest feature member, never through `parent_id`, ancestry, or sibling walking; (3) it is present in the Step 0(c) linked-deliberation snapshot extension — the authoritative skill's Step 0(b) snapshots only manifest task item statuses; linked deliberations are discovered and added to the pre-close declared-status snapshot in Step 0(c) — so it is enumerable and hash-pinned before mutation; and (4) it is individually validated at that same snapshot. `validated_linked_deliberations(S)` never enters `manifest_scope(S)` and never enters `closure_scope(S)` — it never becomes a work item or a delivery member. It DOES enter `required_ids(S)` (per the `required_ids(S)` row above) whenever it is not already truly archived in the pre-close snapshot, exactly like any other `allowed_ids(S)` member that is not the shipment record or a qualifying feature member; this is a postcondition-check participation only, and widens `allowed_ids(S)` by zero *work items* (it is a deliberation/decision artifact, never a task or feature).

#### Invariants

* **INV-1 (Flat closure scope).** `closure_scope(S) = items(S) ∪ {S}`. Ancestry never adds a member. Linked deliberations do not extend `closure_scope(S)`; they are accounted for only in `allowed_ids(S)`.
...
* **INV-6 (Engine-inertness containment gate).** `CASCADE` is permitted only when every artifact in the `parent_id` descendant set the classifier enumerates from each qualifying root feature member — i.e., every artifact transitively reachable from that feature via `parent_id`, exactly the set `classify_shipment_close_path` returns as `out_of_manifest_descendant_ids` — that lies outside `closure_scope(S)` is engine-inert. This gate is scoped precisely to that `parent_id` descendant set and never to `validated_linked_deliberations(S)`: a validated linked deliberation is reached through the engine's linked-deliberation expansion (a feature's `source_deliberation_id`, an embedded description reference, or a referenced deliberation) rather than `parent_id` descent, so it may be live/required and `CASCADE` archiving it is expected and permitted per `required_ids(S)` above — this gate never forces `SAFE_CLOSE` on that account. Engine inertness requires the record's own parsed frontmatter value to satisfy `isinstance(status, str) and status == "archived"`. This is an exact parsed-scalar match: no .lower(), no .strip(), no casefold, no alias table, and no str() coercion may broaden it. Non-string parses such as `status: yes` or a bare `status:` fail closed and force `SAFE_CLOSE`; `Archived`, `ARCHIVED`, and the YAML-quoted literal `status: " archived "` also force `SAFE_CLOSE`. Because the comparison happens after YAML parsing, lexically distinct but YAML-equivalent values that parse to exactly `"archived"` are treated as inert; that is correct because the engine also YAML-parses the field. Any id that resolves to more than one record, whether torn across `queue/` + `archive/` or duplicated within a single root, fails closed and forces `SAFE_CLOSE`; a torn or duplicate out-of-manifest descendant can never satisfy the inertness predicate because its declared status is ambiguous.
* **INV-7 (Baseline invariance replaces baseline presence).** Safe-close records each observed artifact's baseline location plus content hash and requires that nothing outside `closure_scope(S)` changes relative to baseline. An artifact already archived, descoped, or missing at baseline is recorded as baseline state and is explicitly NOT a halt.
...
* **INV-9 (DAG orthogonality).** Shipment sequencing and pipeline topology are orthogonal to closure scope. Closure does not consult the DAG when deciding what this shipment may transition.
* **INV-10 (Postconditions).** After closure, `archived_ids ⊆ allowed_ids(S)`; `required_ids(S) ⊆ archived_ids`; every artifact outside `allowed_ids(S)` is byte-identical to baseline, including `parent_id`; and the shipment record declares `status: archived` plus `archived_status: shipped`.
* **INV-11 (External runtime prerequisite).** Multi-shipment delivery is contract-complete but operationally blocked under backlogit 1.10.1. An intermediate shipment carries no feature member, cannot qualify for `CASCADE`, and is therefore a genuine `SAFE_CLOSE` shipment. If `backlogit move <shipment_id> --status shipped` is refused (exit 9) while the manifest is not cascade-eligible, Ship MUST halt with `RECONCILE_FAIL_NO_SAFE_RECORD_TRANSITION`. The cascade path must NOT be substituted, because `returned_ids` and `parent_id` clearing are indicative and unproven only, and archiving an active shipment record stamps `archived_status: active` rather than `archived_status: shipped`. This is an unresolved upstream tooling gap tracked by durable active stash entry `7F9CB5E9`; no silent workaround, no retry, no hand-edit.
...
|---------|--------------|------------------|----------------------------|
| 1.0.0   | 2026-08-29     | Initial registry | Generated by autoharness   |
| 1.1.0   | 2026-08-29     | Added P-006      | Plan hardening gate enforcement |
| 1.2.0   | 2026-08-29     | Added P-007      | Backlogit archive integrity after shipment |
| 1.3.0   | 2026-08-29     | Added P-008      | Markdown conformance enforcement |
| 1.4.0   | 2026-08-29     | Added P-009      | Merge-commit-only policy |
| 1.5.0   | 2026-08-29     | Added P-010      | Agent role boundary (Stage/Ship/Orchestrator) |
| 1.6.0   | 2026-08-29     | Added P-011      | Branch-before-mutation (Ship) |
| 1.7.0   | 2026-08-29     | Added P-012      | Tool availability and declared degradation |
| 1.8.0   | 2026-08-29     | Added P-013      | Agent tier hierarchy and escalation |
| 1.9.0   | 2026-08-29     | Added P-014      | Local review readiness merge gate — preserves explicit approval and advisory shadow-review migration |
| 1.10.0  | 2026-08-29     | Added P-015      | Single-artifact shipment closure — prohibits cascade shipment-ship, requires verify-after-each invariant + git-revert-on-cascade |
| 1.11.0  | 2026-08-29     | Added P-016      | No parallel branch/worktree execution, with only explicit Stage spike/research worktrees exempt |
| 1.12.0  | 2026-08-29     | Added P-017      | Dark factory autonomy contract — bounded autonomous execution, local-review authority, and audited merge approval |
| 1.13.0  | 2026-08-29     | Added P-018      | Copilot review completion & thread resolution merge gate — deterministic, fail-closed, admin non-bypass, multi-round, bounded-timeout escape |
| 1.14.0  | 2026-08-29     | Added P-019      | Local pre-push quality-gate enforcement — opt-in, single-pass, tool-absent→skip, shift-left counterpart of the CI aggregation gate |
| 1.15.0  | 2026-08-29     | Added P-020      | Post-merge context compaction — mandatory compact-context invocation at Ship closure, invocation-vs-candidate-selection decoupling, skip=violation/closure-incomplete, failed-run non-blocking |
| 1.16.0  | 2026-08-29     | Added P-013.5    | Invocation-time model-routing enforcement — resolve stage/ship role routes with per-sub-field tier fallback, declare-or-degrade invocation directive, fail-closed verification; preserves P-013 tier taxonomy |
| 1.17.0  | 2026-08-29     | Added P-013.6    | Telemetry-driven auto-escalation protocol — escalation-payload contract, config-resolved escalation route with per-field tier3 fallback, canonical ESCALATION_DEGRADED (route/engram unavailable or same-route no-op), authority-preservation invariant; runtime trigger evaluator remains external-guard until a telemetry substrate ships |
| 1.18.0  | 2026-08-29     | Updated P-013.6  | F02FD596: nested per-role escalation hierarchy (`stage.escalation` / `ship.escalation`) with legacy flat `model_routing.escalation` retained as deprecated fallback; both-present (flat + nested) fail-closed as AMBIGUOUS_ESCALATION_CONFIG; ESCALATION_DEGRADED same-route guard now role-scoped |
| 1.19.0  | 2026-08-29     | Updated P-015    | Verified fully-covered-root exception item 7: a pre-archived manifest member does not disqualify the cascade close path — it satisfies coverage/root checks the same as a queued member, does not authorize safe-close fallback, and remains included in the idempotent cascade operation's `archived_ids` result under the unchanged exact-match post-condition |
| 1.20.0  | 2026-08-29     | Added P-021      | Bounded fix-cycle scope containment and deferred expansion capture — C1 same-contract-surface scope test with worked discrimination, C2 mandatory six-field capture (never conditional on a PR or review thread), C3 bounded resolution conditional on thread availability with a symmetric guard, C4 non-bypass (no authorization, including operator authorization, expands the active fix cycle), C5 Ship capture-only carve-out, C6 mandatory Stage deliberation intake |
| 1.21.0  | 2026-08-29     | Corrected P-015  | 155-S: fully-covered-root exception item 7's false full-set-equality claim over `archived_ids` withdrawn and replaced with a two-set `allowed_ids`/`required_ids` gate — `archived_ids` is a transition log (a truly `status: archived` member has no transition to report and is correctly absent), and the live guard is now: fail on `archived_ids - allowed_ids` (unexpected artifact) and fail on `required_ids - archived_ids` (required artifact not archived), evaluated as two separately-labelled, independently-failing conditions. Corrects, and does not delete or edit, the 1.19.0 row above |
| 1.22.0  | 2026-08-29     | Corrected P-015  | 155-S (PR #407 review): item 7's `required_ids` summary now states, identically to the `shipment-reconcile` skill, that the shipment record is a `required_ids` member unconditionally regardless of its own pre-close declared status. Companion skill correction extends Step 0(c)'s classification/snapshot and the two-set gate so `allowed_ids` includes each qualifying feature member's existing, validated linked deliberation IDs (Backlogit's own `collectArchiveCandidateIDs`/`linkedDeliberationIDs` sources), with `required_ids` including such a deliberation only when it was not truly `status: archived` pre-close — preventing a qualifying feature's live linked deliberation from deterministically tripping the unexpected-artifact check after the engine archives it |
| 1.23.0  | 2026-08-29     | Corrected P-015  | 155-S (PR #407 review): item 7's `required_ids` summary and the companion `shipment-reconcile` skill now state that a qualifying feature member is a `required_ids` member unconditionally, exactly like the shipment record — never eligible for the pre-archived tolerance previously (incorrectly) extended to it — because Backlogit's own `ShipShipment` engine forces every explicit qualifying feature member through `status: done` before archive-candidate collection runs, regardless of its own pre-close declared status. Only manifest task items and a qualifying feature member's validated linked deliberations retain the pre-archived tolerance. Additive; does not rewrite the 1.22.0 row above |
| 1.24.0  | 2026-08-29     | Corrected P-015  | 155-S (PR #407 review, thread PRRT_kwDORzpWpM6b2MJv): item 1's superseded hierarchical descendant-coverage precondition now requires walking the feature's FULL descendant tree (every depth via the `parent_id` graph), not only direct children. Backlogit's own `releaseScopeItemIDs` recursively adds every descendant of each manifest item before `collectArchiveCandidateIDs` archives terminal descendants, so a direct-children-only check could wrongly qualify a manifest such as `[feature, task]` when that task itself has an out-of-manifest subtask, letting the destructive cascade archive it before the skill's post-condition gate ever ran. `classify_shipment_close_path` (`src/autoharness/gates/shipment_closure.py`) now builds a full parent/child index and walks it transitively per qualifying-root candidate; regression coverage added in `tests/test_shipment_closure_classification.py`. Corrects, and does not delete or edit, the 1.19.0–1.23.0 rows above |
| 1.25.0  | 2026-09-16     | Rewrote P-015    | 174-S (166-F, operator architectural correction, `docs/decisions/2026-09-15-flat-manifest-shipment-closure-deliberation.md`): withdraws the hierarchical-descendant-coverage premise underlying the 1.19.0–1.24.0 corrections in favor of a flat-manifest, engine-inertness closure model — a qualifying root feature's coverage is now judged against its declared manifest membership plus an exact-parsed-scalar inertness predicate over each out-of-manifest descendant (a descendant is inert only on an exact, case-sensitive `status: archived` match; any other value, absence, or an unresolvable/torn/duplicate/symlinked backlog record fails closed to safe-close), rather than requiring every descendant to itself be an explicit manifest member. `classify_shipment_close_path` (`src/autoharness/gates/shipment_closure.py`) is rewritten accordingly (166.001-T/166.002-T), the `shipment-reconcile` skill's Step 0(c)/Safe-Close/Cascade sections are rewritten to match (166.004-T/166.005-T), and mirror/gate/retrospective coverage is added (166.006-T). Supersedes the hierarchical-closure reading of the 1.19.0–1.24.0 rows above. The 1.24.0 row's substance, attribution, and correction are unchanged; its literal wording was separately normalized in an earlier revision (Copilot review, PR #454, round 6) to replace the withdrawn `"fully-covered"` phrase this file's own contract-consistency tests now prohibit with `"superseded hierarchical descendant-coverage precondition"` — a terminology substitution to keep the historical row internally consistent with this withdrawal, not a rewrite of its correction or a deletion. The 1.19.0–1.23.0 rows remain untouched |
| 1.26.0  | 2026-09-20     | Corrected P-004  | PR #457 (attempt-08 S14/P0 follow-up): the Precondition's global quantifier "expected failure markers in the output for every test function" is withdrawn — read literally it required every test function in the whole historical suite to fail, which is unsatisfiable once a healthy repository has established passing tests and would permanently block any future task. The gate is now scoped explicitly to the current task's generated harness tests: compilation succeeds; the canonical whole-suite test command exits non-zero because those generated harness tests are red; every generated harness test for the current task is discovered and fails with its own expected marker; unrelated, previously established tests may pass. The rejected-outcome list is restated at the policy level (zero-discovery, collection/import/syntax errors, missing/wrong/cross-test markers, and pass/skip/`expectedFailure`/`unexpectedSuccess` of a generated harness test), matching the already-remediated harness-architect actor's Step 5.2 guardrail prose (which is additionally corrected here to name `unexpectedSuccess` alongside `Pass`, the one exact gap found between actor and policy). P-002's role-separation semantics (only harness-architect applies `harness-ready`; ship never bypasses the gate) are unchanged and explicitly restated in the Statement. Coverage: `tests/test_harness_architect_p004_contract.py::PolicyQuantifierCoherenceTests` |
