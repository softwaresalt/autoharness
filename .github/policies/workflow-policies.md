...
**Precondition**: `python -m py_compile src/autoharness/cli.py` exits 0 AND `PYTHONPATH=src python -m unittest discover -s tests` exits non-zero with expected failure markers in the output for every test function.
