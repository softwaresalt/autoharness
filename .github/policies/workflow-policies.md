...
1. Obtain a live G1 approval result over an independent operator channel, matched to the current shipment ID and exact archive paths (G5). Absent, mismatched, ambiguous, timed out, or unreadable → refusal (go to step 5).
2. Run `git restore .backlogit/archive/` to recover the deleted archive files.
3. Re-verify with `git status -- ".backlogit/archive/"` — confirm no deletions remain.
4. Stage the restored files: `git add .backlogit/archive/`. Proceed to the commit step only after verification passes. Record the `APPROVAL: P-007-ARCHIVE-RESTORE` audit comment (G2) after the restore succeeds.
5. If no G1 approval is obtained, or restoration fails after approval, halt and prompt the operator with the exact command above to run themselves (G8). Broadcast a P-005 violation event. Do not restore.
