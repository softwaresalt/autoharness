...
**Required Check (verify-after-each invariant)**: After each safe-close mutation, compare the current state of every observed artifact outside `closure_scope(S)` against its baseline fingerprint. Any change relative to baseline — location change, content-hash change, deletion, rename, new archive presence, or `parent_id` drift — is a cascade signal. The check watches for change; it does not require queue presence at baseline or after each step.
