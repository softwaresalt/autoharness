...
| Shipment lifecycle envelope | Compatible now | Shipment type suffix (`S`), ID format, `custom_fields.items` grouping, 7 MCP tools, and 6 CLI subcommands are proven and production-quality; consume through the backlogit capability pack and registry | Stable artifact type, status lifecycle (queued → blocked → queued for claim-blocking gates, then queued → active → shipped/abandoned) plus archive move, and full CRUD+lifecycle MCP/CLI surface |
