...
Both fields are task-only (features and shipments expose computed, not stored, size
composition; complexity has no feature/shipment-level equivalent). Both are validated
against the enum values defined in `.backlogit/header-def.yaml` before write — invalid
values are rejected fail-closed rather than silently coerced or defaulted.
