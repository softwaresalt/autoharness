...
* **Automatically recommended** when `.graphtor/` is present AND either `mcp_configured` is true or `binary_path` is non-null/non-empty
...
| `_stage.agent.md` | Add graphtor-docs pack guidance block alongside agent-engram guidance |
| `_ship.agent.md` | Add graphtor-docs pack guidance block alongside agent-engram guidance |
...
| `_ship.agent.md` | Step 4.4 review gate invokes adversarial-review agent when pack is active |
...
* `_ship.agent.md` contains the `adversarial-review` conditional at the review gate step
...
* Pack enabled but `_ship.agent.md` does not contain the conditional blocks - re-weave
