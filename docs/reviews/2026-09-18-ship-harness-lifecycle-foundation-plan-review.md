...
terminal_disposition: ADVISORY-REMEDIATED-AWAITING-ATTEMPT-05
terminal_note: "Attempt 04 was terminal for ITS cycle: it reviewed plan revision 5 at content HEAD 4118963a, performed NO remediation, CLOSED both carried findings S10 and S11 on independently re-derived live evidence, and returned ADVISORY on one P2 (S12) and one P3 (S13). The operator has SINCE OPENED A REMEDIATION CYCLE under the standing disposition (fix P2 before publication, carry P3 as a non-blocking follow-up), so attempt 04's terminal-for-cycle designation is WITHDRAWN and this manifest is SUPERSEDED-BY-REMEDIATION. S12 is remediated at its root at plan revision 6 and is now AWAITING INDEPENDENT ATTEMPT 05. S13 is NOT remediated and is carried, by operator disposition, as a low-priority non-blocking follow-up in stash 703B6FAF outside this shipment's scope. BOTH S12 AND S13 REMAIN OPEN: remediation is a DISPOSITION and only a subsequent independent attempt may close a finding. Attempt 04's ADVISORY verdict, its counts and its immutable artifact are UNCHANGED."
awaiting_attempt: 5
...
findings_addressed_pending_review: [S12]
...
| `latest_remediation_revision` | 6 |
| `latest_disposition` | `REMEDIATED-PENDING-REVIEW` |
| `awaiting_attempt` | **05** (remediation cycle open; attempt 04's terminal designation **withdrawn**) |
| `p0_open` / `p1_open` / `p2_open` / `p3_open` | **0 / 0 / 1 / 1** |
| `findings_closed_at_attempt_02` | `S1`–`S6` (all six) |
| `findings_closed_at_attempt_03` | `S7`, `S8`, `S9` |
| `findings_closed_at_attempt_04` | `S10`, `S11` |
| `open_findings` | `S12` (P2), `S13` (P3) — **raised at attempt 04**, both still **open** |
| `findings_addressed_pending_review` | `S12` — remediated at plan revision 6, **not closed** |
...
Performed by **Stage**, not by a reviewer. **Neither finding is closed** and
neither count is decremented: `p2_open` stays **1** and `p3_open` stays **1**
until independent attempt 05 says otherwise. `REMEDIATED-PENDING-REVIEW` is a
**disposition**, never a verdict.
...
**`187-S` remains not publication-eligible**, its tasks remain not claimable,
SM-2's `HARVEST_ADMITTED` stays closed, and the unit remains gated on `188-S`
reaching `shipped`.
...
remediation was performed **at** attempt 04; the `remediation_revision 6` in
its row is the **subsequent Stage remediation cycle** opened under the
operator's standing disposition, which fixed `S12` at its root and carried
`S13` as a non-blocking follow-up. That remediation closes nothing: attempt 05
is awaited.
