...
`telemetry begin` creates a UUIDv4 epoch ID or accepts only a caller-provided
UUID that can be parsed and normalized to the canonical `uuid.UUID(value).hex`
form already generated by the current `ExecutionEpoch` model. Arbitrary
non-empty strings are not valid epoch IDs for the begin/context path. The
canonical ID is lowercase hexadecimal, contains no separators, and is the only
value used to derive the context filename.

The context path is computed from the canonical ID and a workspace-contained
context directory, never from a caller-controlled path fragment. With the default
configuration the artifact path is
`.autoharness/metrics/contexts/{canonical_epoch_id}.json`; when `TelemetryConfig`
points at a custom `database_path`, that database path and the derived
`contexts/` directory must resolve under the workspace root, otherwise telemetry
begin/record rejects the configuration. Implementations must reject traversal,
absolute paths, path separators, symlink escapes, and out-of-root custom
telemetry paths. The command returns at least the canonical `epoch_id`, the
repo-local `context_ref`, and a canonical context digest so the host can persist
...
Context creation is atomic and exclusive. A begin operation writes the new
context without overwriting an existing file (for example exclusive create or an
equivalent same-directory atomic create/rename). If concurrent or repeated begin
calls use the same canonical ID, an existing identical canonical context is
`idempotent_begin`; an existing different context is a conflict and the original
...
metrics. `record --context-ref` validates containment before reading: the
reference must resolve under the configured context directory, the filename stem
must match the canonical epoch ID, the resolved path must not escape through a
symlink, and the stored context digest must match the caller/context metadata
before the context is trusted. No caller-controlled arbitrary path reads are
allowed. `record` then merges the frozen identity/correlation/sizing context with
the close metrics, constructs the `ExecutionEpoch`, and calls the existing record
...
`context_ref` when supplied, the context digest, a distinct canonical epoch
payload digest, and an idempotency outcome such as `created`,
`idempotent_replay`, `partial_repaired`, `conflict_rejected`, or `disabled`.
...
1. A replayable task-close record must provide a canonical UUID epoch ID directly
   or through the pre-execution context. Missing, unparsable, empty, or
...
The context digest and epoch payload digest are intentionally distinct. The
context digest proves the pre-execution identity/correlation/sizing artifact was
not changed; the payload digest proves the close-time `ExecutionEpoch` content is
an identical replay. A matching context digest cannot authorize replacement of a
conflicting epoch payload, and a matching payload digest cannot authorize reading
a context path that fails containment or identity validation.
...
an uncorrelated event mode. When an event supplies `epoch_id`, that value follows
the same canonical UUID normalization rule as `ExecutionEpoch`; `backlog_item_id`
remains a work identifier and is never used as a filename or context path.
...
must obtain a canonical UUID `epoch_id` from the pre-execution context or a
caller-supplied value; task-close helpers must not silently fill a fresh UUID when
the ID is omitted or unparsable. Schema mirrors and parity tests validate the
complete serialized record shape, including the stable identity rule.
...
* **Backlogit hierarchical sizing release gate** is fail-closed for shipment
  ...
  changes. 092-S also includes the begin/context task (`079.014-T`), the
  record/idempotency coordinator task (`079.016-T`), and the Ship lifecycle
  handoff task (`079.015-T`) so the sizing snapshot is captured through a real
  production host path. 079-F is not complete until every 092-S child task is
...
* 079-F planning is ratified, but implementation is paused while shipment 092-S
  ...
  (079.014-T), the record/idempotency task (079.016-T), and Ship lifecycle
  handoff task (079.015-T) are part of that same shipment and must complete
  before 079-F can be marked done.
* Task labels distinguish true external blockers from ordinary dependency
  ordering. Only `079.013-T` carries `blocked-on-external`; generic context and
  idempotency tasks are not labeled external, and shipment 092-S remains the
  machine-enforced implementation gate while the parent feature is blocked.
