...
PR #412 merged after three Copilot review rounds and one independent self-review
round. Four findings were raised and fixed; a fifth was found by self-review
after all three Copilot rounds had passed over it.
...
## The same over-call error recurred twice more — in my own analysis
...
Over-calling and under-calling are the same failure. Both were caught by
re-deriving the number from primary evidence rather than trusting the first
measurement.
...
|---|---|---|---|---|
| `docs/memory` | 36 | 430 KB | no | 0 (0 files older than 14d) |
| `docs/memory/compacted` | 50 | 276 KB | file count only | 0 — compacted outputs, preserved by rule |
...
* `91CE2B66` (chore) — consolidate the 3 plan-review candidates; decide whether
  `-hardening.md` artifacts are in scope for compaction at all
