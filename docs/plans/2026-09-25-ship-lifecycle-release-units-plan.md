...
* **Floor.** `tests/test_harness_noclaim_audit.py` (C5) scans only
  ...
  git and no skip path. C5 lists `harness_read.py`, B1 adds
  `harness_surfaces.py` and D1 `harness_verdict.py`; a listed file
  missing fails.
* **Reading.** Bytes, decoded as strict UTF-8 or the test fails; CR
  ...
  space or, if that has no spanning match and the first ends in `-`,
  with that hyphen and all whitespace at the join dropped, then
