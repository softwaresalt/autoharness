...
| C5 Ship capture-only carve-out | Ship MAY create stash entries **for capture only**. Ship MUST NOT triage, prioritize, re-classify, edit, harvest, deliberate on, or **discretionarily remove** them. Manifest-derived retirement of the source stash entry that fed the shipped scope, after a successful merge (Ship's existing post-merge Step 7), remains **Allowed** — the distinction is provenance, not verb (hardening H2). |
