...
|---|---|
| Workspace lock contention | `REFUSED` terminal state. **Never** auto-break a lock whose PID is live. Stale lock (dead PID or start-time mismatch) still requires explicit `--force-unlock`, and **force-unlock itself must hold the same OS-backed exclusion primitive across inspection AND removal, refusing on any holder mismatch (F31, ruling 3)** — otherwise a lock diagnosed as stale can be re-acquired by a live contender before the delete lands, and the remedy deletes a LIVE holder. |
