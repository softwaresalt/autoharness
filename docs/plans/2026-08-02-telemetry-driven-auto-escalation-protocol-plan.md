...
**Same-route guard (Copilot PR #283 finding).** The tier3 fallback is a *no-op*
for any agent whose role route already equals tier3. Today Stage is
`claude-opus-4.8`/`anthropic`/`high` (`.autoharness/config.yaml:65-68`), which is
exactly `tier3` (`config.yaml:62`); an unset `model_routing.escalation` therefore
resolves Stage to the **identical** model tuple as the failing tier — no actual
escalation to deeper reasoning. The protocol MUST detect this: when the resolved
escalation tuple `(model_family, model_provider, reasoning_effort)` equals the
acting agent's role route, treat the resolution as **`ESCALATION_DEGRADED`** and
fall back to the operator-halt path (do not silently "escalate" to the same
model). An effective auto-escalation requires an explicitly stronger/different
escalation route for such agents. This same-route degradation is a first-class
`ESCALATION_DEGRADED` trigger (defined in T3) and is covered by acceptance in
T7 (detection) and T8 (Stage-case negative test).
...
| T2 | Add `ESCALATION_PROVIDER` / `ESCALATION_FAMILY` / `ESCALATION_REASONING_EFFORT` to the `install-harness/SKILL.md` variable-resolution table with per-field `tier3` fallback, **and add installer gating that installs the new `escalation-protocol.instructions.md` whenever `_stage.agent.md` OR `_ship.agent.md` is installed** (either-agent condition, distinct from the two-agent-only role-enforcement gate at `SKILL.md:1032`) | installer/skill docs | T1 | Variable table lists all three with tier3 fallback semantics; no `{{...}}` left unresolved in the table's own examples; consistent with existing `STAGE_*`/`SHIP_*` rows; installer step installs the escalation-protocol instruction under the either-agent condition |
| T3 | Author the escalation-payload **contract** (fields the halting agent compiles) as a new `templates/instructions/escalation-protocol.instructions.md.tmpl` **installed whenever either agent exists** (NOT `role-enforcement.instructions.md.tmpl`, which installs only in the two-agent model per `install-harness/SKILL.md:1032` and would leave a Stage-only/Ship-only install referencing an uninstalled contract) | instruction template | — | Enumerates payload fields: threshold-kind + count, failure summary, last-N action/observation refs, artifact refs, telemetry-evidence pointers (schema `evidence_path`/`artifact_refs`), resumption checkpoint ref; states authority-preservation invariant; defines `ESCALATION_DEGRADED` **once** (route unavailable OR engram unavailable OR **resolved escalation tuple equals the acting agent's role route — same-route no-op** → fall back to operator-halt) |
| T4 | Extend **Stage** `_stage.agent.md.tmpl` Stop-Conditions/escalation prose: on consecutive-failure threshold → compile payload (T3) → re-route to config-resolved escalation model → terminal engram handoff → halt | agent template (Stage) | T3 | Auto-escalation directive present; dark-mode-safe; cites P-013.6; preserves existing operator-halt behavior when escalation route unavailable **OR resolves to the same tuple as the Stage role route** (`ESCALATION_DEGRADED`) |
...
| T7 | Add fail-closed `verify_workspace.py` checks: `escalation_route_resolution` (route resolves or falls back to tier3, **and flags a same-route resolution — resolved escalation tuple equal to the acting agent's role route — as `ESCALATION_DEGRADED`**) and `escalation_directive_present` (Stage+Ship templates reference the auto-escalation directive + P-013.6, **and the `escalation-protocol` instruction is installed whenever either agent exists**), each gated on file existence | verify CLI | T1, T4, T5 | Both checks registered only when target files exist; unresolved route, same-route no-op, missing directive, or missing either-agent instruction install = verification FAIL (fail-closed); no false-fail on default/legacy installs |
| T8 | Add tests for the T7 checks in `tests/test_verify_workspace.py` | tests | T7 | Positive + negative cases for each check; a missing-escalation-route install with tier3 present passes via fallback; a declared-but-unresolvable route fails; **a Stage install whose resolved escalation tuple equals the Stage role route (same as tier3) fails as `ESCALATION_DEGRADED`** |
...
**Shipment manifest (task-only, per 097-S safe-close contract):** the covering
feature `106-F` is **not** a manifest item — it is derived from each task's
`parent_id`. Manifest `custom_fields.items` lists the nine task IDs only.
**Add order (dependency-topological):** T1 → T2 → T3 → T4 → T5 → T6 → T9 → T7 → T8.
...
- **Same-route escalation no-op** (agent whose role route already equals the
  tier3 fallback — e.g., Stage=`claude-opus-4.8`=tier3 — silently "escalates" to
  the identical model). Mitigation: T3 defines a same-route `ESCALATION_DEGRADED`
  trigger; T7 detects a resolved escalation tuple equal to the acting agent's role
  route and fails closed; T8 pins the Stage same-route negative test. An effective
  auto-escalation requires an explicitly stronger/different escalation route for
  such agents.
