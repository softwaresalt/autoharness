...
An event carrying a non-null `expected_tool` records one explicit expected opportunity for that tool. It is observed only by a correlated invocation event (an event that is not expectation-only) reporting the same `tool_name` within the same correlated event set — an `expected_tool` expectation is satisfied only by a correlated invocation of that same tool for the same logical operation. A status-only expectation record (operation `expect`, status `skipped`) never counts as an invocation by itself. The persisted epoch schema tracks expected/observed/missing tool counts as **flat per-tool maps with no per-operation dimension** — a documented implementation choice — so "the same logical operation" is satisfied by the epoch-level correlation already applied before composition (an expectation and its satisfying invocation both belong to the same correlated epoch/backlog-item event set); within that set, matching is by tool name, mirroring `autoharness.telemetry.gaps.summarize_tool_gaps`. Missing counts clamp at zero and never go negative, even when observed invocations exceed expected opportunities (over-observation). Failed and degraded invocations are counted separately from "missing": a failed or degraded invocation of an expected tool still satisfies that expectation and is never conflated with a tool that was never invoked at all.
