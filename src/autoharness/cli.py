...
Usage:
  ...
  autoharness run               Run a supervised local Copilot CLI session
...
Options:
  --json              Emit the SupervisorResult as JSON instead of a human
                       summary.
  --force-unlock       Opt in to attempting a gated force-unlock recovery
                       when the session lock is refused due to contention.
  --max-restarts N     Restart budget for the supervised child. Default: 0.
  --pty                Spawn the child under a pseudo-terminal (falls back
  ...
  --workspace, -w PATH Workspace root for bootstrap/lock/journal state.
                       ...
                       explicitly (anchored to their own script directory)
...
Exit code is the supervised session's own machine-readable exit code
(see autoharness.supervise.errors.EXIT_CODE_BY_KIND) -- never remapped or
hardcoded by this adapter.
"""


def _parse_run_args(args: list[str]) -> dict:
    """Parse `autoharness run` arguments.

    Every option maps 1:1 onto a ``run_session`` parameter -- this function
    contains no policy of its own beyond argument syntax validation.
    """
    ...
    while index < len(args):
        arg = args[index]
        if arg == "--":
            parsed["child_argv"] = list(args[index + 1 :])
            index = len(args)
            break
        elif arg == "--json":
            parsed["emit_json"] = True
        elif arg == "--force-unlock":
            parsed["force_unlock"] = True
        elif arg == "--max-restarts":
            index += 1
            if index >= len(args):
                raise ValueError("Missing value for --max-restarts")
            try:
                parsed["max_restarts"] = int(args[index])
            except ValueError:
                raise ValueError(f"Invalid --max-restarts value: {args[index]!r}") from None
        elif arg == "--pty":
            parsed["use_pty"] = True
            saw_pty = True
        elif arg == "--no-pty":
            parsed["use_pty"] = False
            saw_no_pty = True
        elif arg == "--session-id":
            index += 1
            if index >= len(args):
            ...
            raise ValueError(f"Unknown run argument: {arg}")
        index += 1
...
def _run_command(args: list[str]) -> None:
    """Run a supervised local Copilot CLI session via `autoharness run`.

    This adapter is pure plumbing: parse argv, construct a real
    :class:`~autoharness.supervise.approvals.ConsoleApprovalService` (the
    ONE place a concrete approval service is instantiated for the CLI
    path), call :func:`autoharness.supervise.app.run_session` with every
    option forwarded 1:1, render the result, and exit with its
