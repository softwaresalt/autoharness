...
**Runtime-binding env variables (120-F post-closure correction,
2026-08-13)**: ``ENGRAM_WORKSPACE``, ``GRAPHTOR_DB_PATH``, and
``GRAPHTOR_SOURCES`` are resolved from ``workspace_root`` by
:func:`_resolve_binding_env` and FORCE-APPLIED (never NO-CLOBBER) via
:func:`_apply_binding_env`. 129-S's own fix removed `.mcp.json`'s broken,
literal, unsubstituted ``${workspaceFolder}`` env overrides and relied on
the spawned Copilot child's ``cwd`` being anchored to ``workspace_root`` so
Engram/graphtor-docs's own CWD-relative defaults would resolve correctly.
That fix is necessary but NOT sufficient: it was directly disproven by a
verified live launch (2026-08-13) whose Copilot-spawned Engram daemon bound
to an entirely different sibling workspace because the Copilot child
process inherited an ALREADY-SET ``ENGRAM_WORKSPACE`` value from its own
ambient process environment -- an explicit environment variable always
wins over a CWD-relative default in both Engram's and graphtor-docs's own
documented `--help` precedence (verified against the actual installed
binaries), regardless of `.mcp.json` or `cwd`. This module now explicitly
constructs and force-applies these three binding variables from the
resolved ``workspace_root`` so the CLI's own explicit ``--workspace``
target ALWAYS wins over any stale ambient/`.env.local`-supplied value for
the same name. ``BACKLOGIT_WORKSPACE`` is deliberately NOT invented here:
`backlogit --help` documents no such environment variable (only a
``--cwd`` flag defaulting to the process's own current working directory),
so backlogit's workspace binding continues to rely exclusively on the
already-anchored child-process ``cwd`` (129-S's original fix).
"""
...
#: The closed, exhaustive set of runtime-binding variable names this module
#: ALWAYS force-applies (never NO-CLOBBER) from the resolved workspace root.
#: See :func:`_resolve_binding_env`/:func:`_apply_binding_env` and the
#: module docstring's "Runtime-binding env variables" section.
_BINDING_ENV_VAR_NAMES: tuple[str, ...] = (
    "ENGRAM_WORKSPACE",
    "GRAPHTOR_DB_PATH",
    "GRAPHTOR_SOURCES",
)
...
class BootstrapResult:
    ...
    Attributes:
        env: The resolved env additions (name -> value) a caller should
            apply directly to a child process environment. For every
            variable EXCEPT :data:`_BINDING_ENV_VAR_NAMES`, this is an
            ADDITION ONLY -- i.e. the variable was not already present (or
            was present-but-empty for the truthy-default variables); it
            never includes a variable that was already set (NO-CLOBBER) or
            that gh resolution left deliberately unset. This INCLUDES
            resolved secret values so a caller can apply them directly.
            :data:`_BINDING_ENV_VAR_NAMES` (``ENGRAM_WORKSPACE``,
            ``GRAPHTOR_DB_PATH``, ``GRAPHTOR_SOURCES``) are the sole
            exception: they are ALWAYS present here with the authoritative
            workspace-derived value, overriding any preset value, per
...
def _resolve_binding_env(workspace_root: Path) -> dict[str, str]:
    """Resolve the AUTHORITATIVE, target-wins runtime-binding env additions.

    Returns a mapping of exactly :data:`_BINDING_ENV_VAR_NAMES` to the
    values derived from ``workspace_root`` (resolved to an absolute,
    normalized path first, so a relative ``--workspace`` argument still
    yields an absolute binding value):

    * ``ENGRAM_WORKSPACE`` -- the resolved workspace root itself.
    * ``GRAPHTOR_DB_PATH`` -- ``<workspace_root>/.graphtor/graph.db``.
    * ``GRAPHTOR_SOURCES`` -- ``<workspace_root>/.graphtor/config/sources.yaml``.

    Verified against the actual installed binaries' own documented
    ``--help`` output (2026-08-13): ``engram --help`` documents
    ``--workspace`` defaulting to ``[env: ENGRAM_WORKSPACE=]``;
    ``graphtor-docs --help`` documents ``--config``/``[env:
    GRAPHTOR_SOURCES=]`` and ``--db-path``/``[env: GRAPHTOR_DB_PATH=]``.
    ``backlogit --help`` documents NO analogous environment variable --
    only a ``--cwd`` flag defaulting to the process's own current working
    directory -- so no ``BACKLOGIT_WORKSPACE`` variable is invented here;
    backlogit's workspace binding continues to rely exclusively on the
    already-anchored child-process ``cwd`` (129-S's original fix; see
    ``sidecar.py`` and ``app.py``'s ``_default_child_process_factory``).
    Every value stays inside ``workspace_root`` -- no path escapes the
    workspace, and no secret or credential is ever placed in this mapping.
    """

    resolved_root = Path(workspace_root).resolve()
    return {
        "ENGRAM_WORKSPACE": str(resolved_root),
        "GRAPHTOR_DB_PATH": str(resolved_root / ".graphtor" / "graph.db"),
        "GRAPHTOR_SOURCES": str(resolved_root / ".graphtor" / "config" / "sources.yaml"),
    }


def _apply_binding_env(
    workspace_root: Path,
    working_env: MutableMapping[str, str],
    resolved: MutableMapping[str, str],
    warnings: list[str],
) -> None:
    """Force-apply the authoritative runtime-binding env additions.

    Unlike every other value this module resolves (``.env.local``
    NO-CLOBBER loads, ``COPILOT_HOME``/``ENGRAM_DATA_DIR`` directory
    defaults, ``gh``-resolved tokens), :data:`_BINDING_ENV_VAR_NAMES` are
    NEVER NO-CLOBBER: the explicitly resolved ``workspace_root`` this
    supervisor was invoked with ALWAYS wins over any stale/ambient/
    ``.env.local``-supplied value for the same name, because these three
    variables are cross-workspace BINDING identifiers, not per-workspace
    configuration details -- ``.env.local`` may still configure
    non-binding details (e.g. an embedding model or data directory) but
    cannot redirect the supervisor's Engram/graphtor-docs children to a
    different workspace.

    This directly closes the runtime defect verified live 2026-08-13: a
    real launch's Copilot-spawned Engram daemon bound to an entirely
    different sibling workspace (``C:\\Source\\GitHub\\engram``) because
    the Copilot child process inherited an ALREADY-SET ``ENGRAM_WORKSPACE``
    value from its own ambient process environment -- proving that 129-S's
    fix (removing `.mcp.json`'s broken ``${workspaceFolder}`` literal and
    anchoring the child's own ``cwd``) is NECESSARY but NOT SUFFICIENT: an
    explicit environment variable always wins over a CWD-relative default
    in both Engram's and graphtor-docs's own documented precedence,
    ...
    for name, authoritative_value in _resolve_binding_env(workspace_root).items():
        ...
        working_env[name] = authoritative_value
        resolved[name] = authoritative_value
...
def bootstrap_workspace(
    ...
    *,
    ...
    Returns a :class:`BootstrapResult` whose ``env`` field carries the
    ...
    constructing. Every entry is an addition EXCEPT
    :data:`_BINDING_ENV_VAR_NAMES` (``ENGRAM_WORKSPACE``,
    ``GRAPHTOR_DB_PATH``, ``GRAPHTOR_SOURCES``), which are ALWAYS present in
    ``env`` regardless of any preset value -- see
    :func:`_apply_binding_env`'s docstring for why these three are
    deliberately exempt from the NO-CLOBBER contract every other resolved
    variable follows.
    """
    ...
    # 120-F post-closure correction (2026-08-13): force-apply the
    # authoritative ENGRAM_WORKSPACE/GRAPHTOR_DB_PATH/GRAPHTOR_SOURCES
    # runtime-binding variables LAST, so they win over anything loaded
    # above from `.env.local` or already present in the caller's baseline
    # environment. See `_apply_binding_env`'s docstring for the verified
    # live defect this closes.
    _apply_binding_env(workspace_root, working_env, resolved, warnings)
