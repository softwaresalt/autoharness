...
def _record_from_sqlite_row(row: sqlite3.Row) -> dict[str, Any]:
    ...
    economics = {
        # Copilot review r3c NEW-1: absent or NULL v1.1 columns must reconstruct as
        # ``None`` so ExecutionEpoch.from_mapping marks them unavailable via
        # provenance instead of persisting a false observed zero. A ``0`` default
        # here defeated the provenance fallback for legacy/partial SQLite rows.
        "input_tokens": _row_get(row, "input_tokens"),
        "output_tokens": _row_get(row, "output_tokens"),
        "cogs_usd": _row_get(row, "cogs_usd"),
        "duration_seconds": _row_get(row, "duration_seconds"),
        "cached_input_tokens": _row_get(row, "cached_input_tokens"),
        "cumulative_input_tokens": _row_get(row, "cumulative_input_tokens"),
        "cumulative_output_tokens": _row_get(row, "cumulative_output_tokens"),
        "context_tokens_before": _row_get(row, "context_tokens_before"),
        "context_tokens_after": _row_get(row, "context_tokens_after"),
        "context_area_tokens": _row_get(row, "context_area_tokens"),
        "avoided_read_estimated_tokens": _row_get(row, "avoided_read_estimated_tokens"),
        "tool_output_estimated_tokens": _row_get(row, "tool_output_estimated_tokens"),
    ...
    operations = {
        ...
        "routed_lookup_count": _row_get(row, "routed_lookup_count"),
        "raw_file_read_count": _row_get(row, "raw_file_read_count"),
        "raw_search_count": _row_get(row, "raw_search_count"),
        "avoided_file_read_count": _row_get(row, "avoided_file_read_count"),
        "tool_output_bytes": _row_get(row, "tool_output_bytes"),
        "expected_tool_count": _row_get(row, "expected_tool_count"),
        "observed_expected_tool_count": _row_get(row, "observed_expected_tool_count"),
        "missing_expected_tool_count": _row_get(row, "missing_expected_tool_count"),
        ...
        "degraded_tool_count": _row_get(row, "degraded_tool_count"),
        "stale_or_unavailable_index_count": _row_get(row, "stale_or_unavailable_index_count"),
    ...
    outcome = {
        ...
        "tool_failure_count": _row_get(row, "tool_failure_count"),
        "tool_degraded_count": _row_get(row, "tool_degraded_count"),
        "tool_gap_count": _row_get(row, "tool_gap_count"),
